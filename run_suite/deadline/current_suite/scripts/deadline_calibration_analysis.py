...
def append_report(report_path: Path, summaries: list[dict], recommended: dict | None) -> None:
    ...
    for row in summaries:
        lines.append(
            ...
            f"degradation moderate expiry={row['degradation_moderate_expired_rate_mean']} (p95 lag={row['degradation_moderate_completion_lag_p95_seconds_mean']}s), "
...
def main(argv: list[str]) -> int:
    ...
    for row in summaries:
        report_lines.append(
            f"- {row['deadline_window_seconds']}s: baseline in-time={row['baseline_in_time_rate_mean']}, baseline p95 lag={row['baseline_completion_lag_p95_seconds_mean']}s, "
            f"degradation moderate expiry={row['degradation_moderate_expired_rate_mean']}, body-phase expiry={row['body_phase_expired_rate_mean']}, "
